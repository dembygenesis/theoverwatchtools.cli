...
func (dm *DockerManager) UpsertContainer(ctx context.Context, recreate bool) (string, error) {
	...
	for _, ctn := range allContainers {
		hasContainerNameCollision := utility.WalkSlice(ctn.Names, func(name string) bool {
		...
		hasPortBindingCollision := utility.WalkSlice(ctn.Ports, func(port types.Port) bool {
