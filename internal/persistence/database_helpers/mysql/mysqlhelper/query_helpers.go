...
func preparePagination(ctx context.Context, db *sqlx.DB, query string, pagination *model.Pagination, args ...interface{}) (string, error) {
	...
	totalPages := int(math.Ceil(float64(totalCount) / float64(pagination.MaxRows)))
	...
	offset := (pagination.Page - 1) * pagination.MaxRows

	paginatedQuery := query + " LIMIT " + strconv.Itoa(pagination.MaxRows) + " OFFSET " + strconv.Itoa(offset)
	...
	if err = pagination.ValidatePagination(); err != nil {
		return "", fmt.Errorf("validate bachir: %v", err)
	}
