...
import (
	"context"
	"fmt"
	"github.com/dembygenesis/local.tools/internal/lib/docker_manager"
...
type MariaDBConfig struct {
	...
	ExposedInternalPort int    `json:"exposed_internal_port" mapstructure:"exposed_internal_port" validate:"required,int_greater_than_zero"`
	ExposedExternalPort int    `json:"exposed_external_port" mapstructure:"exposed_external_port" validate:"required,int_greater_than_zero"`
}

func (m *MariaDBConfig) Validate() error {
	return utility.ValidateStruct(m)
}
...
func NewMariaDB(cfg *MariaDBConfig) (*docker_manager.DockerManager, error) {
	...
	dm, err := docker_manager.NewDockerManager(&docker_manager.ContainerConfig{
