...
func (o CreateMode) Validate() error {
	...
	if ok := utils_common.Contains(modes, func(v CreateMode) bool {
		return o == v
	}); !ok {
...
// New creates a new database with modes:
//
...
func New(ctx context.Context, c *ConnectionParameters, opts *NewOpts) (*sqlx.DB, error) {
	var (
		db        *sqlx.DB
		errPrefix string
		err       error
...
func recreateDatabase(ctx context.Context, c *ConnectionParameters) (*sqlx.DB, error) {
	db, err := GetClient(ctx, &ClientOptions{
