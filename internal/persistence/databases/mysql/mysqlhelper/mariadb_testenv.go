...
func testSpawnMariaDB(t *testing.T, cp *mysqlutil.ConnectionSettings) (*sqlx.DB, *mysqlutil.ConnectionSettings, func(ignoreErrors ...bool)) {
	m.Lock()
	defer m.Unlock()

	// Set database logs onn for testing
	boil.DebugMode = true
...
func testExistingMariaDB(t *testing.T, cp *mysqlutil.ConnectionSettings) (*sqlx.DB, *mysqlutil.ConnectionSettings, func(ignoreErrors ...bool)) {
	db, err := NewDbClient(context.TODO(), &ClientOptions{
		ConnString: cp.GetConnectionString(true),
		Close:      false,
	})
...
func TestGetMockMariaDB(t *testing.T) (*sqlx.DB, *mysqlutil.ConnectionSettings, func(ignoreErrors ...bool)) {
